...
def get_cars(request, dealer_id):
    cars = get_request(f"/cars/{dealer_id}")
    return JsonResponse(cars, safe=False)


def get_dealerships(request, state="All"):
    endpoint = "/fetchDealers" if state == "All" else f"/fetchDealers/{state}"
    dealers = get_request(endpoint) or []
...
def cars_by_make(request, dealer_id, make):
    cars = get_request(f"/carsbymake/{dealer_id}/{make}") or []
    return JsonResponse(cars, safe=False)

def get_inventory(request, dealer_id):
    cars = get_request(f"/cars/{dealer_id}") or []
    return JsonResponse({"status": 200, "cars": cars})

def cars_by_model(request, dealer_id, model):
    return JsonResponse(
        get_request(f"/carsbymodel/{dealer_id}/{model}"),
        safe=False
    )

def cars_by_year(request, dealer_id, year):
    return JsonResponse(
        get_request(f"/carsbyyear/{dealer_id}/{year}"),
        safe=False
    )

def cars_by_mileage(request, dealer_id, mileage):
    return JsonResponse(
        get_request(f"/carsbymaxmileage/{dealer_id}/{mileage}"),
        safe=False
    )

def cars_by_price(request, dealer_id, price):
    return JsonResponse(
        get_request(f"/carsbyprice/{dealer_id}/{price}"),
        safe=False
    )
